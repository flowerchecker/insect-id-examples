...
def create_identification(file_names):
    # More optional parameters: https://insect.kindwise.com/docs
    payload = {
    ...
    response = requests.post(
        'https://insect.kindwise.com/api/v1/identification?async',
        json=payload,
        headers=headers,
    )
...
def get_completed_identification(access_token, tries=10):
    ...
    params = {
        # Details docs: https://insect.kindwise.com/docs
        'details': 'common_names,gbif_id,taxonomy,rank',
        'language': 'en,de'
    }
    for _ in range(tries):
        ...
        response = requests.get(
            f'https://insect.kindwise.com/api/v1/identification/{access_token}?' + urlencode(params),
            headers=headers,
        )
...
if __name__ == '__main__':
    access_token = create_identification(
        [
            BASE_DIR / 'images' / 'unknown_insect.jpg',
        ]
    )
